    ...
    def get(self, request, format=None):
        ...
        if id != None:
            ...
            if len(plan) > 0:
                ...
                return Response(data, status=status.HTTP_200_OK)
            return Response({'Bad Request': 'Invalid Plan ID'}, status=status.HTTP_404_NOT_FOUND)
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            if id != None:
                ...
                print("no id provided")
                plan = Plan(title=title, description=description, start_time=start_time, end_time=end_time)
                plan.save()
